...
public interface ClassroomRepository extends JpaRepository<Classroom, Integer> {
	...
	@Query("SELECT DISTINCT c FROM Classroom c WHERE c.building.idbuilding=:build AND c.idclassroom NOT IN (SELECT l.classroom.idclassroom FROM TypeLesson l WHERE l.classroom.building.idbuilding=:build AND (l.start<=:start AND l.end>=:end) OR (l.start>=:start AND l.end>=:end) OR (l.start<=:start AND l.end<=:end) AND l.day.idday=:day AND l.scheduler.term.idterm=:idterm AND l.scheduler.term.academicYear.idacademicYear=:idaa)")
	public List<Classroom> findAvailableClassesByBuild(@Param("build")int build, @Param("idaa")int idacademicyear, @Param("idterm")int idterm, @Param("start")Date start, @Param("end")Date end, @Param("day")int day);
